  ...
  <div>
    <el-col class="fs875 mT10 cababab artlClass">
      <el-col
        :span="8"
        v-if="item[key]"
        v-for="(val, key) in nameContrastIndex"
      >
        <!--申请日-->
        <span v-if="key == 'applyDay'"
          >{{ val }}：{{ item[key] | formatDate("YY-MM-DD") }}</span
        >
...
export default {
  ...
  methods: {
    ...
    nameContrastFn(_mediaName = this.mediaName) {
      ...
      nameIndex
        ? (_this.nameContrastIndex = nameIndex.contrast)
        : (_this.nameContrastIndex = "");
    }
  },
  mounted() {
    let _this = this;
